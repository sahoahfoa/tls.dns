package digitalocean

import (
	"time"

	"github.com/caddyserver/caddy/v2"
	"github.com/caddyserver/caddy/v2/modules/caddytls"
	tlsdns "github.com/caddyserver/tls.dns"
	"github.com/go-acme/lego/v3/challenge"
	"github.com/go-acme/lego/v3/providers/dns/digitalocean"
)

func init() {
	caddy.RegisterModule(DigitalOcean{})
}

// CaddyModule returns the Caddy module information.
func (DigitalOcean) CaddyModule() caddy.ModuleInfo {
	return caddy.ModuleInfo{
		ID:  "tls.dns.digitalocean",
		New: func() caddy.Module { return new(DigitalOcean) },
	}
}

// DigitalOcean configures a solver for the ACME DNS challenge.
type DigitalOcean struct {
	// An authentication token from your account.
	AuthToken string `json:"auth_token,omitempty"`

	tlsdns.CommonConfig
}

// NewDNSProvider returns a DNS challenge solver.
func (wrapper DigitalOcean) NewDNSProvider() (challenge.Provider, error) {
	cfg := digitalocean.NewDefaultConfig()
	if wrapper.AuthToken != "" {
		cfg.AuthToken = wrapper.AuthToken
	}
	if wrapper.CommonConfig.BaseURL != "" {
		cfg.BaseURL = wrapper.CommonConfig.BaseURL
	}
	if wrapper.CommonConfig.TTL != 0 {
		cfg.TTL = wrapper.CommonConfig.TTL
	}
	if wrapper.CommonConfig.PropagationTimeout != 0 {
		cfg.PropagationTimeout = time.Duration(wrapper.CommonConfig.PropagationTimeout)
	}
	if wrapper.CommonConfig.PollingInterval != 0 {
		cfg.PollingInterval = time.Duration(wrapper.CommonConfig.PollingInterval)
	}
	if wrapper.CommonConfig.HTTPClient != nil {
		cfg.HTTPClient = wrapper.CommonConfig.HTTPClient.HTTPClient()
	}
	return digitalocean.NewDNSProviderConfig(cfg)
}

// Interface guard
var _ caddytls.DNSProviderMaker = (*DigitalOcean)(nil)
